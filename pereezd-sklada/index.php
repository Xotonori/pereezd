<?php include_once  ($_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'header' . DIRECTORY_SEPARATOR . 'header-part1.php'); ?>
<meta name="description" itemprop="description" content="Компания Переезд без Хлопот оказывает полный комплекс услуг по организации складских переездов в Санкт-Петербурге и области." />
<meta name="keywords" itemprop="keywords" content="переезд склада" />
<title>Переезд склада в Санкт-Петербурге - Переезд без хлопот</title>
<link rel="canonical" href="https://pereezdy-spb.ru/pereezd-sklada/"/>
<?php include_once  ($_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'header' . DIRECTORY_SEPARATOR . 'header-part2.php'); ?>
<img class="sofa" data-src="/images/header/sofa.png" alt="диван">
<img class="man" data-src="/images/header/man.png" alt="грузчик">
<?php include_once  ($_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'header' . DIRECTORY_SEPARATOR . 'header-part3.php'); ?>
<h1>Переезд склада</h1>
<div class="urgent-call">Срочный вызов за <span class="red-bold">15 минут</span></div>
<button type="button" class="request-call" data-toggle="modal" data-target="#header-modal">Заказать переезд склада</button>
<?php include_once  ($_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'header' . DIRECTORY_SEPARATOR . 'header-part4.php'); ?>



<main>
    <!--////PAGE-DESCRIPTION/////////////////////////////////////////////////////////////////////////////////////////////-->
    <div class="container">
        <div class="row d-flex justify-content-center">
            <div class="col-12 js-long-text container how-much-description">
                <div class="row">
                    <p class="col-12 paragraph">
                        Наши сотрудники по первому звонку отправятся к вам, чтобы перевезти склад в Питере и его пригородах. Мы берем на себя организацию всех этапов работы, а вам предоставляем гарантии и полезные рекомендации о том, как сделать последствия переезда исключительно положительными.
                    </p>
                    <div class="col d-flex justify-content-end">
                        <button class="js-podrobno">
                            <span>Подробно</span>
                            <img data-src="/images/header/city-triangle.png" alt="треугольник">
                        </button>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-12 text-center">
                <h2>Тарифы на грузоперевозки по Санкт-Петербургу и области</h2>
            </div>
            <table class="tariff">
                <tbody>
                <tr class="tariff-title">
                    <td rowspan="2" width="153"><b>Автомобиль</b></td>
                    <td colspan="3" width="388" valign="top"><b>Тарифы по Санкт-Петербургу и ЛО</b></td>
                </tr>
                <tr>
                    <td width="93" valign="top"><b>Мин. Заказ</b></td>
                    <td width="91" valign="top"><b>По </b><b>Санкт-Петербургу</b></td>
                    <td width="69" valign="top"><b>За КАД*</b></td>
                </tr>
                <tr>
                    <td width="153">
                        <p style="text-align: left;"><a href="/cars#car1">Газель-фургон</a> (10м3/1,5 тонн)</p>
                    </td>
                    <td width="93" valign="top">3 час + 1 час подача</td>
                    <td width="91" valign="top">450 руб/час</td>
                    <td width="69" valign="top">16 руб/км</td>
                </tr>
                <tr>
                    <td width="153">
                        <p style="text-align: left;"><a href="/cars#car2">Газель Next</a> (18м3/1,5 тонн)</p>
                    </td>
                    <td width="93" valign="top">4 час + 1 час подача</td>
                    <td width="91" valign="top">500 руб/час</td>
                    <td width="69" valign="top">18 руб/км</td>
                </tr>
                <tr>
                    <td width="153">
                        <p style="text-align: left;"><a href="/cars#car3">Газель удлиненная</a> (22,9м3/2,5 тонн)</p>
                    </td>
                    <td width="93" valign="top">5 час + 1 час подача</td>
                    <td width="91" valign="top">550 руб/час</td>
                    <td width="69" valign="top">20 руб/км</td>
                </tr>
                <tr>
                    <td width="153">
                        <p style="text-align: left;"><a href="/cars/">MAN</a> (36м3/5 тонн)</p>
                    </td>
                    <td width="93" valign="top">5 час + 1 час подача</td>
                    <td width="91" valign="top">800 руб/час</td>
                    <td width="69" valign="top">27 руб/км</td>
                </tr>
                <tr>
                    <td width="153">
                        <p style="text-align: left;"><a href="/cars/">MAN</a> (42м3/5 тонн)</p>
                    </td>
                    <td width="93" valign="top">6 час + 1 час подача</td>
                    <td width="91" valign="top">850 руб/час</td>
                    <td width="69" valign="top">28 руб/км</td>
                </tr>
                <tr>
                    <td width="153">
                        <p style="text-align: left;"><a href="/cars#car6">МАN</a> (48м3/10 тонн)</p>
                    </td>
                    <td width="93" valign="top">7 час + 1 час подача</td>
                    <td width="91" valign="top">950 руб/час</td>
                    <td width="69" valign="top">30 руб/км</td>
                </tr>
                </tbody>
            </table>
        </div>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-12 text-center">
                <h3 class=_header-3>Тарифы на услуги грузчиков</h3>
            </div>
            <table class="tariff">
                <tbody>
                <tr class="tariff-title">
                    <td width="308" valign="top"><b>Виды работ</b></td>
                    <td width="188" valign="top"><b>Цена за работу 1 грузчика</b></td>
                    <td width="92" valign="top"><b>Мин. заказ</b></td>
                </tr>
                <tr>
                    <td width="308">Погрузка, разгрузка и складские работы</td>
                    <td width="188">От 300 до 350 руб в час</td>
                    <td width="92">5 часов</td>
                </tr>
                <tr>
                    <td width="308">Разборка, упаковка, погрузка, разгрузка, сборка, расстановка, демонтаж, монтаж (карнизы, люстры и т.д.)</td>
                    <td width="188">От 350 до 450 руб</td>
                    <td width="92">4 часа</td>
                </tr>
                <tr>
                    <td width="308">Такелажные работы&nbsp;(предметы свыше 100 кг:сейфы,&nbsp;пианино, банкоматы и т.п.)</td>
                    <td colspan="2" width="280">Стоимость рассчитывается индивидуально!</td>
                </tr>
                </tbody>
            </table>
            <div class="row additional-info1 d-flex justify-content-center mt-0">
                <div class="col-11">
                    КАД* - Выезд за КАД, оплата в обе стороны. Все цены на услугу "переезд под ключ" в нашей компании рассчитываются индивидуально после БЕСПЛАТНОГО выезда оценщика, а также приблизительную стоимость Вашего переезда вы можете узнать у наших менеджеров по телефону в Санкт-Петербурге: +7 (812) 982-88-58.
                </div>
            </div>
        </div>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-12 text-center">
                <h3 class=_header-3>С нами хорошо, потому что:</h3>
            </div>
            <div class="col-12">
                <ul class="marker">
                    <li>Мы не просто возим вещи - мы понимаем специфику работы каждого типа склада: на перевозку магазинных, промышленных, музейных складов мы направляем работников с опытом именно в данной сфере.</li>
                    <li>На всех этапах работы ваше имущество в безопасности - мы письменно гарантируем сохранность вещей, работаем с проверенными страховщиками.</li>
                    <li>Наше ценообразование прозрачно, а расценки – индивидуальны: какой бы ни была задача, мы непременно договоримся!</li>
                </ul>
            </div>
        </div>
    </div>
    <!--////DISCOUNT/////////////////////////////////////////////////////////////////////////////////////////////////-->
    <div class="container-fluid header-main-theme discount-block mb-5">
        <div class="row justify-content-center">
            <div class="col-12 d-flex">
                <img class="rullon mx-auto" data-src="/images/rullon.png" alt="рулон">
            </div>
            <h6 class="_header-6 col-11 text-center"><span class="important">Закажите услугу</span> по переезду магазина и получите скидку!</h6>
            <span class="additional-info2 col-11 text-center mt-2">Скидка на упаковочный материал <br><span class="big-red">40%</span></span>
            <div class="col-12 d-flex">
                <!-- Modal -->
                <button type="button" class="request-call mx-auto" data-toggle="modal" data-target="#header-modal">Заказать переезд</button>
            </div>
        </div>
    </div>
    <div class="container mb-4">
        <div class="row">
            <div class="col-12 text-center">
                <h3 class=_header-3>Индивидуальный план работ</h3>
            </div>
            <p class="col-12 paragraph pt-0">
                Главная особенность переезда склада - огромный объем вещей, что требует слаженной работы целой команды специалистов. Поэтому мы сразу разрабатываем план-график переезда. Во-первых, это позволяет точно оценить стоимость наших работ (при переезде под ключ данная услуга бесплатна), заключить договор и предоставить необходимые документальные гарантии. Во-вторых, работа по плану существенно сокращает время, позволяет слаженно работать даже внушительному количеству сотрудников, заранее найти лучшие пути подъезда и места погрузки, использовать лучшее время для транспортировки без пробок, для максимальной работы в светлое время суток и так далее. Кроме того, так ваши представители на любом этапе смогут проконтролировать работы.
            </p>
        </div>
    </div>
    <div class="container mb-4">
        <div class="row">
            <div class="col-12 text-center">
                <h3 class=_header-3>Учет и каталогизация</h3>
            </div>
            <p class="col-12 paragraph pt-0">
                Переезд склада, особенно крупного - отличная возможность для внеплановой инвентаризации вашего имущества. Первым этапом любой работы мы ставим тщательную каталогизацию любых имеющихся в распоряжении вашей компании вещей. Ни одна коробка, ни одна стойка, ни один стул не отправятся в путь, пока их функция, принадлежность и место не будут нанесены на упаковку и внесены в каталог. Воспользовавшись созданной нами системой, вы сможете не только проверить сохранность имущества, но и использовать получившийся каталог для разработки оптимальной организации пространства на новом месте.
            </p>
        </div>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-12 text-center">
                <h3 class=_header-3>Самая комплексная услуга</h3>
            </div>
            <p class="col-12 paragraph pt-0">
                "Переезд без Хлопот" предоставляет, пожалуй, самый полный комплекс услуг по перевозке склада в Санкт-Петербурге. Мы будем рады, если вы доверите все работы нам, но мы готовы подключиться на любом из этапов:
            </p>
            <div class="col-12">
                <ul class="marker">
                    <li>разработка проекта перевозки;</li>
                    <li>демонтаж оборудования и складских систем, оргтехники;</li>
                    <li>упаковка любого имущества, маркировка, каталогизация;</li>
                    <li>упаковка всего имущества - в том числе, с помощью наших материалов, предоставляемых с 40% скидкой;</li>
                    <li>погрузка, транспортировка, экспедирование, разгрузка;</li>
                    <li>монтаж оборудования, расстановка мебели;</li>
                    <li>уборка помещения после завершения работ.</li>
                </ul>
                <p>И на каждом этапе мы сделаем все необходимое, чтобы на новом месте ваша работа стала эффективнее и удобнее!</p>
            </div>
        </div>
    </div>
    <!--////photos////////////////////////////////////////////////////////////////////////////////////////////////-->
    <h2 class="_header-3 text-center">Фото</h2>
    <div class="container client-reviews">
        <div class="row">
            <div class="carousel-container mx-auto">
                <div id="client-reviews" class="carousel slide mx-auto mt-4 mb-4">
                    <div class="carousel-inner">
                        <div class="carousel-item active">
                            <a href="/images/pereezd-sklada/photo1.jpg" data-toggle="lightbox" data-gallery="review-photo">
                                <img class="d-block" data-src="/images/pereezd-sklada/photo1_mini.jpg" alt="Фото 1">
                            </a>
                        </div>
                        <div class="carousel-item">
                            <a href="/images/pereezd-sklada/photo2.jpg" data-toggle="lightbox" data-gallery="review-photo">
                                <img class="d-block" data-src="/images/pereezd-sklada/photo2_mini.jpg" alt="Фото 2">
                            </a>
                        </div>
                        <div class="carousel-item">
                            <a href="/images/pereezd-sklada/photo3.jpg" data-toggle="lightbox" data-gallery="review-photo">
                                <img class="d-block" data-src="/images/pereezd-sklada/photo3_mini.jpg" alt="Фото 3">
                            </a>
                        </div>
                        <div class="carousel-item">
                            <a href="/images/pereezd-sklada/photo4.jpg" data-toggle="lightbox" data-gallery="review-photo">
                                <img class="d-block" data-src="/images/pereezd-sklada/photo4_mini.jpg" alt="Фото 4">
                            </a>
                        </div>
                        <div class="carousel-item">
                            <a href="/images/pereezd-sklada/photo5.jpg" data-toggle="lightbox" data-gallery="review-photo">
                                <img class="d-block" data-src="/images/pereezd-sklada/photo5_mini.jpg" alt="Фото 5">
                            </a>
                        </div>
                        <div class="carousel-item">
                            <a href="/images/pereezd-sklada/photo6.jpg" data-toggle="lightbox" data-gallery="review-photo">
                                <img class="d-block" data-src="/images/pereezd-sklada/photo6_mini.jpg" alt="Фото 6">
                            </a>
                        </div>
                        <div class="carousel-item">
                            <a href="/images/pereezd-sklada/photo7.jpg" data-toggle="lightbox" data-gallery="review-photo">
                                <img class="d-block" data-src="/images/pereezd-sklada/photo7_mini.jpg" alt="Фото 7">
                            </a>
                        </div>
                        <div class="carousel-item">
                            <a href="/images/pereezd-sklada/photo8.jpg" data-toggle="lightbox" data-gallery="review-photo">
                                <img class="d-block" data-src="/images/pereezd-sklada/photo8_mini.jpg" alt="Фото 8">
                            </a>
                        </div>
                        <div class="carousel-item">
                            <a href="/images/pereezd-sklada/photo9.jpg" data-toggle="lightbox" data-gallery="review-photo">
                                <img class="d-block" data-src="/images/pereezd-sklada/photo9_mini.jpg" alt="Фото 9">
                            </a>
                        </div>
                        <div class="carousel-item">
                            <a href="/images/pereezd-sklada/photo10.jpg" data-toggle="lightbox" data-gallery="review-photo">
                                <img class="d-block" data-src="/images/pereezd-sklada/photo10_mini.jpg" alt="Фото 10">
                            </a>
                        </div>
                    </div>
                    <ol class="carousel-indicators mx-auto">
                        <li data-target="#client-reviews" data-slide-to="0" class="active"></li>
                        <li data-target="#client-reviews" data-slide-to="1"></li>
                        <li data-target="#client-reviews" data-slide-to="2"></li>
                        <li data-target="#client-reviews" data-slide-to="3"></li>
                        <li data-target="#client-reviews" data-slide-to="4"></li>
                        <li data-target="#client-reviews" data-slide-to="5"></li>
                        <li data-target="#client-reviews" data-slide-to="6"></li>
                        <li data-target="#client-reviews" data-slide-to="7"></li>
                        <li data-target="#client-reviews" data-slide-to="8"></li>
                        <li data-target="#client-reviews" data-slide-to="9"></li>
                    </ol>
                </div>
            </div>
        </div>
    </div>

    <!--////RECOMMENDATION/////////////////////////////////////////////////////////////////////////////////////////////-->
    <div class="recommendation container">
        <div class="row d-flex justify-content-center">
            <div class="col-12 text-center">
                <h2 class="_header-3">Нас<span class="important"> рекомендуют</span></h2>
            </div>
        </div>
    </div>
    <div class="bd-example">
        <div id="recommendationCaption1" class="carousel slide" data-ride="carousel">
            <div class="carousel-inner">
                <div class="carousel-item active">
                    <img data-src="/images/ofisnyy-pereezd/libher.png" class="d-block mx-auto" alt="слайд">
                    <div class="carousel-caption d-none d-md-block">
                        <h5>Коновалов Е.А.<br><small>Начальник отдела</small></h5>
                        <p class="text-left text-lg-center">
                            Все работы были выполнены профессионально, на высоком уровне и в оговоренные сроки. Сотрудники компании компетентны и вежливы, ни один предмет при переезде офиса не пострадал.
                        </p>
                        <a href="/images/client-reviews/84882121_199.jpg" data-toggle="lightbox" data-gallery="gallery">посмотреть отзыв</a>
                    </div>
                </div>
                <div class="carousel-item">
                    <img data-src="/images/ofisnyy-pereezd/megafon.png" class="d-block mx-auto" alt="слайд">
                    <div class="carousel-caption d-none d-md-block">
                        <h5>Калугин А.И.<br><small>Административный директор</small></h5>
                        <p class="text-left text-lg-center">
                            ОАО "Мегафон Ритейл" в лице административного директора Калугина А.И. выражает благодарность ООО "Трансдокер" за организацию переезда нашей компании.
                        </p>
                        <a href="/images/client-reviews/Megafon_212.jpg" data-toggle="lightbox" data-gallery="gallery">посмотреть отзыв</a>
                    </div>
                </div>
                <div class="carousel-item">
                    <img data-src="/images/ofisnyy-pereezd/casamia.png" class="d-block mx-auto" alt="слайд">
                    <div class="carousel-caption d-none d-md-block">
                        <h5>Кормилина Н.А.<br><small>Директор по персоналу</small></h5>
                        <p class="text-left text-lg-center">
                            ООО «КазаМиа» в лице директора по персоналу Кормилиной Н.А. выражает благодарность ООО «Трансдокер» за качественную работу по организации переезда нашего офиса.
                        </p>
                        <a href="/images/client-reviews/Kaza_Mia_214.jpg" data-toggle="lightbox" data-gallery="gallery">посмотреть отзыв</a>
                    </div>
                </div>
                <div class="carousel-item">
                    <img data-src="/images/ofisnyy-pereezd/raskom.png" class="d-block mx-auto" alt="слайд">
                    <div class="carousel-caption d-none d-md-block">
                        <h5>Ганза Д.А.<br><small>Директор Дирекции по продажам</small></h5>
                        <p class="text-left text-lg-center">
                            ЗАО «РАСКОМ» выражает Вам свое почтение и благодарность за отличную организацию и оказание услуг по переезду офиса.
                        </p>
                        <a href="/images/client-reviews/Raskom_216.jpg" data-toggle="lightbox" data-gallery="gallery">посмотреть отзыв</a>
                    </div>
                </div>
                <div class="carousel-item">
                    <img data-src="/images/ofisnyy-pereezd/mobius.png" class="d-block mx-auto" alt="слайд">
                    <div class="carousel-caption d-none d-md-block">
                        <h5>Харченко Н.А.<br><small>Генеральный директор</small></h5>
                        <p class="text-left text-lg-center">
                            Компания "Переезд без хлопот" все очень быстро оценили, организовали и переезд превратился из "второго пожара" в приятные хлопоты. Рады, что заказали услуги именно у вас.
                        </p>
                        <a href="/images/client-reviews/MOBIUS_220.jpg" data-toggle="lightbox" data-gallery="gallery">посмотреть отзыв</a>
                    </div>
                </div>
                <div class="carousel-item">
                    <img data-src="/images/ofisnyy-pereezd/ghelamco.png" class="d-block mx-auto" alt="слайд">
                    <div class="carousel-caption d-none d-md-block">
                        <h5>Стивен Уилсон<br><small>Генеральный директор</small></h5>
                        <p class="text-left text-lg-center">
                            Компания Геламко в процессе переезда в новый офис сотрудничала с компанией ООО "Трансдокер" и выражает ей благодарность за осуществление переезда под ключ в максимально короткий срок.
                        </p>
                        <a href="/images/client-reviews/Gelamko_205.jpg" data-toggle="lightbox" data-gallery="gallery">посмотреть отзыв</a>
                    </div>
                </div>
            </div>
            <a class="_carousel-control carousel-control-prev" href="#recommendationCaption1" role="button" data-slide="prev">
                <img data-src="/images/left-arrow-type1.png" alt="стрелка влево">
                <span class="sr-only">Previous</span>
            </a>
            <a class="_carousel-control carousel-control-next" href="#recommendationCaption1" role="button" data-slide="next">
                <img data-src="/images/right-arrow-type1.png" alt="стрелка вправо">
                <span class="sr-only">Next</span>
            </a>
        </div>
    </div>
</main>
<?php include_once ($_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'footer' . DIRECTORY_SEPARATOR . 'footer.php'); ?>

