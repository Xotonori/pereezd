<?php include_once  ($_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'header' . DIRECTORY_SEPARATOR . 'header-part1.php'); ?>
    <meta name="description" itemprop="description" content="Предлагаем услуги складов временного хранения товаров и вещей в Санкт-Петербурге. Выгодные цены. Адреса складов." />
    <meta name="keywords" itemprop="keywords" content="склад временного хранения,цена,стоимость" />
    <title>Склады временного хранения в Санкт-Петербурге</title>
    <link rel="canonical" href="https://pereezdy-spb.ru/kvartirnyy-pereezd/"/>
<?php include_once  ($_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'header' . DIRECTORY_SEPARATOR . 'header-part2.php'); ?>
    <img class="sofa" src="/images/header/sofa.png" alt="диван">
    <img class="man" src="/images/header/man.png" alt="грузчик">
<?php include_once  ($_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'header' . DIRECTORY_SEPARATOR . 'header-part3.php'); ?>
    <h1>Склады временного хранения</h1>
    <div class="urgent-call">Срочный вызов за <span class="red-bold">15 минут</span></div>
    <button type="button" class="request-call" data-toggle="modal" data-target="#header-modal">Заказать временное хранение</button>
<?php include_once  ($_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'header' . DIRECTORY_SEPARATOR . 'header-part4.php'); ?>

<main>
    <div class="container">
        <div class="row">
            <div class="col-12">
                <p class="mt-5">У нас можно арендовать чистый и сухой склад временного хранения в Санкт-Петербурге, где можно оставить личные вещи, мебель, бытовую технику, другое имущество на время ремонта, отпуска или переезда. Компания оказывает клиентам полный комплексный сервис, предоставляя возможность заказать исполнение работ «под ключ» или выбрать необходимый набор услуг.</p>
                <h2 class="text-center">Преимущества</h2>
                <ul class="marker">
                    <li>Оставляя вещи на нашем складе временного хранения товаров, вы можете быть совершенно уверены в их сохранности.</li>
                    <li>В зависимости от типа имущества клиенты могут арендовать отапливаемое помещение или открытую площадку.</li>
                    <li>Территория находится под круглосуточной охраной, контролируется системой видеонаблюдения.</li>
                    <li>В отапливаемых помещениях поддерживается оптимальный для длительного хранения мебели, книг, бытовой техники уровень температуры и влажности.</li>
                    <li>Комплекс оборудован системой автоматического пожаротушения.</li>
                    <li>Арендаторы имеют круглосуточный доступ к своему имуществу.</li>
                </ul>
                <h4>Адрес склада временного хранения:</h4>
                <span>ул. Глиняная, 5 (м. пл. Александра Невского II)</span>
                <table class="mt-4">
                    <tbody>
                    <tr class="tariff-title">
                        <td>Площадь</td>
                        <td>3 дня</td>
                        <td>Неделя</td>
                        <td>Месяц</td>
                        <td>3 месяца</td>
                        <td>6 месяцев</td>
                        <td>Год</td>
                    </tr>
                    <tr>
                        <td>1 м<sup>3</sup></td>
                        <td>330</td>
                        <td>490</td>
                        <td>1 100</td>
                        <td>3 300</td>
                        <td>6 410</td>
                        <td>12 280</td>
                    </tr>
                    <tr>
                        <td>1 м<sup>2</sup></td>
                        <td>330</td>
                        <td>490</td>
                        <td>1 100</td>
                        <td>3 300</td>
                        <td>6 410</td>
                        <td>12 280</td>
                    </tr>
                    <tr>
                        <td>1,5 м<sup>2</sup></td>
                        <td>500</td>
                        <td>730</td>
                        <td>1 650</td>
                        <td>4 950</td>
                        <td>9 610</td>
                        <td>18 420</td>
                    </tr>
                    <tr>
                        <td>2 м<sup>2</sup></td>
                        <td>660</td>
                        <td>970</td>
                        <td>2 200</td>
                        <td>6 600</td>
                        <td>12 810</td>
                        <td>24 560</td>
                    </tr>
                    <tr>
                        <td>2,5 м<sup>2</sup></td>
                        <td>830</td>
                        <td>1 210</td>
                        <td>2 750</td>
                        <td>8 250</td>
                        <td>16 010</td>
                        <td>30 690</td>
                    </tr>
                    <tr>
                        <td>3 м<sup>2</sup></td>
                        <td>990</td>
                        <td>1 450</td>
                        <td>3 300</td>
                        <td>9 900</td>
                        <td>19 210</td>
                        <td>36 830</td>
                    </tr>
                    <tr>
                        <td>4 м<sup>2</sup></td>
                        <td>1 320</td>
                        <td>1 930</td>
                        <td>4 400</td>
                        <td>13 200</td>
                        <td>25 610</td>
                        <td>49 110</td>
                    </tr>
                    <tr>
                        <td>4,5 м<sup>2</sup></td>
                        <td>1 490</td>
                        <td>2 170</td>
                        <td>4 950</td>
                        <td>14 850</td>
                        <td>28 810</td>
                        <td>55 250</td>
                    </tr>
                    <tr>
                        <td>5 м<sup>2</sup></td>
                        <td>1 650</td>
                        <td>2 410</td>
                        <td>5 500</td>
                        <td>16 500</td>
                        <td>32 010</td>
                        <td>61 380</td>
                    </tr>
                    <tr>
                        <td>6 м<sup>2</sup></td>
                        <td>1 980</td>
                        <td>2 890</td>
                        <td>6 600</td>
                        <td>19 800</td>
                        <td>38 420</td>
                        <td>73 660</td>
                    </tr>
                    <tr>
                        <td>7 м<sup>2</sup></td>
                        <td>2 310</td>
                        <td>3 370</td>
                        <td>7 700</td>
                        <td>23 100</td>
                        <td>44 820</td>
                        <td>85 940</td>
                    </tr>
                    <tr>
                        <td>8 м<sup>2</sup></td>
                        <td>2 640</td>
                        <td>3 850</td>
                        <td>8 800</td>
                        <td>26 400</td>
                        <td>51 220</td>
                        <td>98 210</td>
                    </tr>
                    <tr>
                        <td>9 м<sup>2</sup></td>
                        <td>2 970</td>
                        <td>4 340</td>
                        <td>9 900</td>
                        <td>29 700</td>
                        <td>57 620</td>
                        <td>110 490</td>
                    </tr>
                    <tr>
                        <td>10 м<sup>2</sup></td>
                        <td>3 300</td>
                        <td>4 820</td>
                        <td>11 000</td>
                        <td>33 000</td>
                        <td>64 020</td>
                        <td>122 760</td>
                    </tr>
                    <tr>
                        <td>15 м<sup>2</sup></td>
                        <td>4 950</td>
                        <td>7 220</td>
                        <td>16 500</td>
                        <td>49 500</td>
                        <td>96 030</td>
                        <td>184 140</td>
                    </tr>
                    <tr>
                        <td>16 м<sup>2</sup></td>
                        <td>5 280</td>
                        <td>7 700</td>
                        <td>17 600</td>
                        <td>52 800</td>
                        <td>102 440</td>
                        <td>196 420</td>
                    </tr>
                    </tbody>
                </table>
                <h4>Адрес склада временного хранения:</h4>
                <span>ул. Капитанская, Д. 4. «МОРСКОЙ ФАСАД»</span>
                <table class="mt-4">
                    <tbody>
                    <tr class="tariff-title">
                        <td>Площадь<br>
                            бокса, м2</td>
                        <td>Цена за<br>
                            месяц, при оплате за 1 месяц</td>
                        <td>Цена за<br>
                            месяц, при оплате за 1 квартал</td>
                        <td>Цена за месяц,<br>
                            при оплате за 0,5 год</td>
                        <td>Цена за месяц,<br>
                            при оплате за 1 год</td>
                    </tr>
                    <tr>
                        <td> 0,8</td>
                        <td> 1200</td>
                        <td> 1200</td>
                        <td> 1100</td>
                        <td> 1100</td>
                    </tr>
                    <tr>
                        <td> 1,3</td>
                        <td> 2100</td>
                        <td> 2000</td>
                        <td> 1900</td>
                        <td> 1800</td>
                    </tr>
                    <tr>
                        <td> 2,3</td>
                        <td> 3500</td>
                        <td> 3400</td>
                        <td> 3200</td>
                        <td> 3000</td>
                    </tr>
                    <tr>
                        <td> 2,4</td>
                        <td> 3600</td>
                        <td> 3500</td>
                        <td> 3300</td>
                        <td> 3100</td>
                    </tr>
                    <tr>
                        <td> 2,6</td>
                        <td> 3900</td>
                        <td> 3800</td>
                        <td> 3600</td>
                        <td> 3400</td>
                    </tr>
                    <tr>
                        <td> 2,9</td>
                        <td> 4400</td>
                        <td> 4200</td>
                        <td> 4000</td>
                        <td> 3800</td>
                    </tr>
                    <tr>
                        <td> 3,2</td>
                        <td> 4500</td>
                        <td> 4300</td>
                        <td> 4100</td>
                        <td> 3900</td>
                    </tr>
                    <tr>
                        <td> 3,3</td>
                        <td> 4700</td>
                        <td> 4500</td>
                        <td> 4300</td>
                        <td> 4000</td>
                    </tr>
                    <tr>
                        <td> 3,4</td>
                        <td> 4800</td>
                        <td> 4600</td>
                        <td> 4400</td>
                        <td> 4100</td>
                    </tr>
                    <tr>
                        <td> 3,5</td>
                        <td> 4900</td>
                        <td> 4700</td>
                        <td> 4500</td>
                        <td> 4200</td>
                    </tr>
                    <tr>
                        <td> 3,6</td>
                        <td> 5100</td>
                        <td> 4900</td>
                        <td> 4600</td>
                        <td> 4400</td>
                    </tr>
                    <tr>
                        <td> 3,7</td>
                        <td> 5200</td>
                        <td> 5000</td>
                        <td> 4700</td>
                        <td> 4500</td>
                    </tr>
                    <tr>
                        <td> 3,8</td>
                        <td> 5400</td>
                        <td> 5200</td>
                        <td> 4900</td>
                        <td> 4600</td>
                    </tr>
                    <tr>
                        <td> 3,9</td>
                        <td> 5500</td>
                        <td> 5300</td>
                        <td> 5000</td>
                        <td> 4700</td>
                    </tr>
                    <tr>
                        <td> 4</td>
                        <td> 5600</td>
                        <td> 5400</td>
                        <td> 5100</td>
                        <td> 4800</td>
                    </tr>
                    <tr>
                        <td> 4,2</td>
                        <td> 5900</td>
                        <td> 5700</td>
                        <td> 5400</td>
                        <td> 5100</td>
                    </tr>
                    <tr>
                        <td> 4,3</td>
                        <td> 6100</td>
                        <td> 5800</td>
                        <td> 5500</td>
                        <td> 5200</td>
                    </tr>
                    <tr>
                        <td> 4,4</td>
                        <td> 6200</td>
                        <td> 5900</td>
                        <td> 5600</td>
                        <td> 5300</td>
                    </tr>
                    <tr>
                        <td> 4,5</td>
                        <td> 6300</td>
                        <td> 6000</td>
                        <td> 5700</td>
                        <td> 5400</td>
                    </tr>
                    <tr>
                        <td> 4,6</td>
                        <td> 6500</td>
                        <td> 6200</td>
                        <td> 5900</td>
                        <td> 5600</td>
                    </tr>
                    <tr>
                        <td> 5,2</td>
                        <td> 6800</td>
                        <td> 6500</td>
                        <td> 6200</td>
                        <td> 5800</td>
                    </tr>
                    <tr>
                        <td> 5,5</td>
                        <td> 7200</td>
                        <td> 6900</td>
                        <td> 6500</td>
                        <td> 6200</td>
                    </tr>
                    <tr>
                        <td> 6,1</td>
                        <td> 8000</td>
                        <td> 7600</td>
                        <td> 7200</td>
                        <td> 6800</td>
                    </tr>
                    <tr>
                        <td> 7</td>
                        <td> 9100</td>
                        <td> 8700</td>
                        <td> 8200</td>
                        <td> 7800</td>
                    </tr>
                    <tr>
                        <td> 7,3</td>
                        <td> 9500</td>
                        <td> 9100</td>
                        <td> 8600</td>
                        <td> 8100</td>
                    </tr>
                    <tr>
                        <td> 8,6</td>
                        <td> 11200</td>
                        <td> 10700</td>
                        <td> 10100</td>
                        <td> 9600</td>
                    </tr>
                    <tr>
                        <td> 16,1</td>
                        <td> 19400</td>
                        <td> 18500</td>
                        <td> 17500</td>
                        <td> 16500</td>
                    </tr>
                    <tr>
                        <td> 17,3</td>
                        <td> 20800</td>
                        <td> 19800</td>
                        <td> 18800</td>
                        <td> 17700</td>
                    </tr>
                    <tr>
                        <td> 17,5cc</td>
                        <td> 21000</td>
                        <td> 20000</td>
                        <td> 18900</td>
                        <td> 17900</td>
                    </tr>
                    </tbody>
                </table>
                <h4>Адресa складов:</h4>
                <span>ул. Сызранская, д. 23, лит. А</span><br>
                <span>6-ой верхний пер., д. 12, стр. Б</span>
                <table class="mt-4">
                    <tbody>
                    <tr class="tariff-title">
                        <td>Хранилище</td>
                        <td>Цена 1 м²</td>
                        <td>3 месяца<br>скидка 7%</td>
                        <td>6 месяцев<br>скидка 10%</td>
                        <td>12 месяцев<br>скидка 15%</td>
                    </tr>
                    <tr>
                        <td>
                            С-Петербург, Сызранская ул., д. 23, лит. А
                        </td>
                        <td>
                            1400 руб/мес
                        </td>
                        <td>
                            <span>1302</span>&nbsp;руб/мес
                        </td>
                        <td>
                            <span>1260</span>&nbsp;руб/мес
                        </td>
                        <td>
                            <span>1190</span>&nbsp;руб/мес
                        </td>
                    </tr>
                    <tr>
                        <td>
                            С-Петербург, 6-й верхний пер., д. 12, стр. Б
                        </td>
                        <td>
                            1400 руб/мес
                        </td>
                        <td>
                            <span>1302</span>&nbsp;руб/мес
                        </td>
                        <td>
                            <span>1260</span>руб/мес
                        </td>
                        <td>
                            <span>1190</span>&nbsp;руб/мес
                        </td>
                    </tr>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
    <!--////QUESTION/////////////////////////////////////////////////////////////////////////////////////////////-->
    <div class="person-page">
        <div class="container-fluid header-main-theme person-page-price">
            <h3 class="_header-3 mt-3 text-center"><span class="important">Задайте вопрос</span><br>специалисту</h3>
            <img class="back-office" src="/images/back-office.png" alt="диван">
        </div>
        <div class="person-page-call">
            <img class="man mx-auto" src="/images/manager.png" alt="менеджер">
            <img class="man double-quotes mx-auto" src="/images/double-quotes.png" alt="менеджер">
            <h6 class="_header-6 text-center">
                Николай Денисов
            </h6>
            <div class="additional-info2 text-center mt-2">
                Специализация: переезды
                по Санкт-Петербургу "под ключ"<br>
                +7 (812) 982-88-58
            </div>
            <div class="col-11 js-long-text container how-much-description">
                <div class="row">
                    <p class="col-12 paragraph text-left">
                        У нашей компании вы можете выбрать склад временного хранения вещей, наилучшим образом подходящий по вместимости, расположению и стоимости. Мы заинтересованы в долговременном сотрудничестве, поэтому ввели дифференцированные тарифы для разных сроков аренды.
                    </p>
                    <div class="col d-flex justify-content-center">
                        <button class="js-podrobno">
                            <span>Подробно</span>
                            <img src="/images/header/city-triangle.png" alt="треугольник">
                        </button>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12 d-flex">
                        <button type="button" class="request-call mx-auto" data-toggle="modal" data-target="#questionModal1">Задать вопрос</button>
                    </div>
                    <div class="modal fade" id="questionModal1" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="questionModal1-title">Заказать квартирный переезд</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <form action="" class="js-modal" id="questionProfile1__info">
                                    <div class="modal-body">
                                        <div class="form-group">
                                            <label for="question1-name">Ваше имя</label>
                                            <input type="text" class="form-control" name="name" data-id="name" id="question1-name"
                                                   placeholder="Ваше имя">
                                            <small class="text-danger"></small>
                                        </div>
                                        <div class="form-group">
                                            <label for="question1-tel">Ваш телефон</label>
                                            <input type="tel" class="form-control" name="tel" data-id="tel" id="question1-tel"
                                                   placeholder="+79210000000">
                                            <small class="text-danger"></small>
                                        </div>
                                        <div class="form-group">
                                            <label for="question1-question">Ваш вопрос</label>
                                            <textarea class="form-control" name="question" data-id="question" id="question1-question"
                                                      placeholder="Ваш вопрос?"></textarea>
                                            <small class="text-danger"></small>
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="submit" class="send-button">Отправить</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</main>
<?php include_once ($_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'footer' . DIRECTORY_SEPARATOR . 'footer.php'); ?>